...
st.write("Extracts ONLY encircled/checked letters from Google Drive folder.")
...
# --- ITO NA YUNG GUSTO MO, WALA NANG UPLOAD ---
st.markdown("### GOOGLE DRIVE LINK HERE:")
drive_link = st.text_input(
    "Paste mo dito yung GDrive folder link kung saan mo gusto i-extract:",
        ...
        if st.button("🚀 I-EXTRACT NA FROM DRIVE LINK", type="primary"):
            ...
            try:
                if ftype == "folder":
                    ...
                    files = res.get('files', [])
                    st.info(f"Nakita sa folder: {len(files)} files - Paired as 1+2 = 1 Row")
                    for f in files:
